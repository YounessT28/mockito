...
public class PersonService implements IEntityService<PersonDTO> {
    ...
    public boolean update(int id, String name) throws ExceptionService
    {
        String surname = "test", mail = "test", address = "test", phone = "034930", firstname = "test", password = "momo", subjectTaught = "cours";
        String methodName = new Object(){}.getClass().getEnclosingMethod().getName();
        double average = 12.50; Date dateofbirth = new Date(System.currentTimeMillis());
        int role = 4;
        int res = 0;
        PersonDTO studentDTO = new StudentDTO(surname, mail, address, phone, 0, firstname,password,dateofbirth,average);
        Person student = StudentMapper.dtoToBusiness((StudentDTO)studentDTO);
